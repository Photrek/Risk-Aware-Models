...
class LitVaeRnnModel(pl.LightningModule):
    ...
    def forward(self, batch):
        img_seq = batch   # size = [batch_size, seq_len, Nx, Ny]
        ...
        if 1:  # process sequence all-at-once.
            unrolled_all = torch.zeros(seq_len-1, cfg.vae_rnn.BATCH_SIZE, self.vae.feature_volume, 
                device = img_seq.device)
            for i_seq in range(seq_len-1):
                ...
                #unrolled = encoded.view(-1, self.vae.feature_volume).unsqueeze(0)
                unrolled_all[i_seq] = encoded.view(-1, self.vae.feature_volume)
            h_latentMost, _  = self.lstm(unrolled_all)
            new_unrolled = self.lstm_readout(h_latentMost)
            #new_meanseq[:,i_seq] = self.vae.q_mean(new_unrolled)
            new_meanseq = self.vae.q_mean(new_unrolled).movedim(0,1)
            #new_logvarseq[:,i_seq] = self.vae.q_logvar(new_unrolled)
            new_logvarseq = self.vae.q_logvar(new_unrolled).movedim(0,1)
            # Following conducts sampling.
            new_z = self.vae.z(new_meanseq, new_logvarseq)
            new_z_projected = self.vae.project(new_z).view(
                cfg.vae_rnn.BATCH_SIZE,
                -1,    # this dim will be auto filled in since the others are set
                self.vae.kernel_num,
            ...
            #recon_img = self.vae.decoder(new_z_projected[:,0])
            for i_seq in range(seq_len-1):
                imgseq_reconstructed[:,i_seq] = self.vae.decoder(new_z_projected[:,i_seq]).squeeze()

        else: # step through each image individually
            ...
            new_logvarseq = torch.zeros(
                img_seq.size(0), seq_len-1, self.vae.z_size,
                device = img_seq.device)
            hc_vec = None  # initialize

            #print("check one-pass approach"); set_trace()
            for i_seq in range(seq_len-1):
                # Note: in following, unsqueeze goes from [32, 28, 28] --> [32, 1, 28, 28]
                encoded = self.vae.encoder(img_seq[:,i_seq,:,:].unsqueeze(1))
                ...
                if 0:  # This assumes LSTM migrates the mean and logvar temporally.
                    ...
                    unrolled = encoded.view(-1, self.vae.feature_volume).unsqueeze(0)
                    # Note: unrolled.size = [1, bs, self.vae.feature_volume], typically [1, 16, 1568]
                    # The "1" is to hold the sequence dimension.
                    #set_trace()
                    if hc_vec is None:  # first pass
                        #h_vec, unused_cell = self.lstm(unrolled)
                        #h_latentMost, hc_vec = self.lstm(unrolled)
                        h_latentMost, hc_vec = self.lstm(unrolled)
                    else:
                        h_latentMost, hc_vec = self.lstm(unrolled, hc_vec)
                    ...
                    new_unrolled = self.lstm_readout(h_latentMost[0,:])
                    new_meanseq[:,i_seq] = self.vae.q_mean(new_unrolled)
                    new_logvarseq[:,i_seq] = self.vae.q_logvar(new_unrolled)
                    #new_z = self.vae.z(new_meanseq[:,i_seq], new_logvarseq[:,i_seq])


                # TODO: create an option to do all seq at once: 
                # encode, unroll, put in seq, then process that sequence all at once
